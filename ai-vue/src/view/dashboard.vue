<template>
    <div class="dashboard-container">
        <el-row :gutter="20">
            <!-- el-row：行组件, :gutter="20"：网格间距为20px -->
            <el-col :span="6">
                <!-- el-col：列组件, :span="6"：占6个网格宽度 -->
                ...
                <el-card v-if="aiData.systemOverview">
                    <div class="card-content">
                        ...
                        <div class="info">
                            ...
                            <p class="subtitle-title">活跃用户：{{ aiData.systemOverview.todayNewSessions }}</p>
                        </div>
                    </div>
                </el-card>
            </el-col>
...
const initEmotionChart = () =>{
    ...
    //如果dom节点不存在，直接返回
    if(emotionChart){
        //判断当前实例是否存在，存在销毁当前的图表
        emotionChart.dispose()
    }
    //创建echarts实例
    emotionChart =echarts.init(emotionChartRef.value)
    // 通过获取当前emotionChartRef.value实例给到echarts的init方法里面进行初始化，返回一个实例对象emotionChart
    ...
    const option = {
        ...
        tooltip: {
            //弹窗的触发类型（axis：坐标轴触发，item：数据项触发，none：不触发弹窗）
            trigger:'axis',
            borderColor:'#fab1a0',
            //弹窗窗的边框颜色,下面的属性是边框宽度
            borderWidth:1,
        ...
        xAxis:{
            ...
            // map里面需要返回一个数组，一个一元数组，每一个元素都是一个横坐标的值
            axisLine:{
                lineStyle:{
                    color:'#2d3436'
                }
